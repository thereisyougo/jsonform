...
(function ($, window) {
	...
	function setFormItemValues(jsonObject) {
		...
		for (var key in items) {
			try {
				...
				setItemValue.call(this, key, val);
			} catch (e) {
				continue;
			}
		}
	...
	function getFormItemValues() {
		...
		for (var key in items) {
			// 对定义了的回传字段进行封闭
			if (items[key].datafield)
				jsonObject[items[key].datafield] = getItemValue.call(this, key);
			else
				jsonObject[key] = getItemValue.call(this, key);
		}
		return jsonObject;
	}
	...
	function validate(obj) {
		...
		for (var key in items) {
			...
			val = getItemValue.call(this, item.mapping);
	 ...
	 * 内部使用
	 */
	function getItemValue(mapping) {
		var item = this.options.items[mapping],
			val;
	 ...
	 * 内部使用
	 */
	function clearItemValue(mapping) {
		var item = this.options.items[mapping],
			dom = item.dom;
		//log(item);
	 ...
	 * 内部使用
	 */
	function setItemValue(mapping, val) {
		var item = this.options.items[mapping];
		if (item.type === 'radio') {
			$('input[name=' + item.id + '][value="' + val + '"]').prop('checked', true);
		} else if (item.type === 'checkbox') {
			$('input[name=' + item.id + ']').each(function (index, item) {
				if (val.length > 1) {
					for (var i = 0, len = val.length; i < len; ++i) {
						$(item).filter('[value="' + val[i] + '"]').prop('checked', true);
					}
				...
				try {
					...
					if ($.type(props.items) === 'array') {
						for (var i = 0, len = props.items.length; i < len; ++i) {
							...
							self.options.items[item.mapping] = cmp;
						}
					}
